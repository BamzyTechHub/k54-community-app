...
- ~~**New, confirmed blocker**: lesson/topic/quiz *detail* has no working path yet.~~ **RESOLVED 2026-07-20 — this is a capability wall, not an enrollment wall.** Tested every `tutor/v1` sub-resource against course 791 ("K54 Global Growth Program") with the same working JWT: `courses/{id}`, `course-contents/{id}`, `topics?course_id=`, `author-information/{id}` **all return the identical `403 rest_forbidden "Sorry, you are not allowed to do that."`**, regardless of which specific resource is requested. That uniformity is the tell: if this were enrollment-gated, at least the course's own public metadata (`course-rating`, `author-information`) would likely succeed while curriculum-specific routes failed - instead every single `tutor/v1` endpoint fails identically, which is the signature of a single capability check (e.g. `tutor_instructor`/`manage_options`) applied uniformly at the namespace level, not a per-resource enrollment check. Also checked for a `wp/v2` post-type workaround (the trick that unlocked the course catalog itself) - none exists (`lp_course`/`lp_quiz` are LearnPress's, not Tutor's; no `lesson`/`topic`/`quiz` post type is registered for Tutor at all), and the course object (`wp/v2/courses/{id}?_embed`) doesn't embed curriculum data. **Conclusion: this needs a WordPress-side change** (granting this account an instructor-level capability, or reconfiguring Tutor LMS's REST permission callback to allow enrolled students) - there is no "enroll via API and unlock it" path for the Flutter app to build against as-is. Moved from 🟡 to 🔴 in `roadmap.md`.
