...
- ~~Voice notes & message forwarding — CONFIRMED real (route existence only)~~ **Full request/response shapes now confirmed (2026-07-20, disposable-message live test against thread 72, cleaned up after):**
  - **Forward:** `POST /better-messages/v1/message/{message_id}/forward` body `{"thread_ids": [id, ...]}` → `{"result": true, "sent": {"<threadId>": <newMessageId>}, "errors": []}`.
  - **Pin/unpin a single message:** `POST /better-messages/v1/thread/{id}/pinMessage` / `.../unpinMessage`, body `{"messageId": <int>}` (camelCase — NOT `message_id`, that returns a misleading `403 rest_forbidden "Message not found"`). Returns the message id back (pin) / bare `true` (unpin).
  - **Delete messages:** `POST /better-messages/v1/thread/{id}/deleteMessages`, body `{"messageIds": [<int>, ...]}` (camelCase plural — singular/snake_case variants either 403 or crash the server with an uncaught `TypeError` in `bp-better-messages-websocket/inc/rest-api.php:786`, so those shapes must never be sent). Response: `{"deleted": [...], "errors": [...]}`.
  - **Voice notes are a two-step flow**, not a direct upload-with-message: (1) `POST /better-messages/v1/thread/{id}/upload`, multipart field name `file` → `{"result": bool, "error": "", "id": <attachment_id>}` (the confirmed test file was a synthetic silent WAV, which came back `"result": false` with an empty error string but still a valid usable `id` — the false likely reflects a server-side duration/waveform-analysis step failing on a non-real audio file, not the upload itself); (2) `POST /better-messages/v1/thread/{id}/sendVoice` body `{"attachment_id": <id from step 1>}` (`temp_id`/`temp_time` optional, same optimistic-send convention as `send`) → full envelope with `messages[0].message` as an HTML string (`<div class="bpbm-voice-message" data-message="{audioUrl}">...`) and the same audio URL duplicated at `messages[0].meta.files[0].url` (cleaner parse target than the HTML).
  - `sendMessage`'s response was also found to be non-empty after all (contradicting the earlier "empty body" note above): `{"result": true, "message_id": <int>, "thread_id": <int>, "redirect": false}` — worth reconsidering the "never trust the send response" caution now that a real shape exists, though re-fetching the thread afterward remains harmless either way.
  - **Generic file/image attachment (not voice) — CONFIRMED live 2026-07-20**, same disposable-message-and-delete discipline: two-step, same `thread/{id}/upload` (field `file`) as voice notes, then `POST thread/{id}/send` body `{"message": "<!-- BM-ONLY-FILES -->", "files": [attachment_id], "temp_id", "temp_time"}`. Tried `attachment_id`/`attachments`/`attachment_ids` first — all returned `200` with no error but silently attached nothing (no new message created, or an empty `meta.files`); only the plural `files` key actually worked. This is now wired in the app (chat composer's attach/camera icons, image_picker only - arbitrary non-image files still need a `file_picker` package).
